...
\begin{frame}
    ...
    \speakernote[item]{be brief, concise and clear}
    \speakernote[item]{put emphasis on ``\emph{diverse} fuzzers''}
\end{frame}
...
\script{Let's begin by looking at different fuzzing techniques}
...
\begin{frame}
    ...
    \script<1>{%
        ...
        was logged into his office's terminal via a dial-up modem. The heavy
    ...
    \speakernote<1>[item]{was logged into his office terminal}
    \speakernote<1>[item]{heavy rain causing noise on the line}
    \speakernote<1>[item]{noise on line cause inputs to scramble and crash}
...
\end{lstlisting}
    \script{%
        ...
        execution reach that portion of code; to achieve this, the input needs
        to satisfy a number of checks that the target imposes:
        \begin{itemize}
    ...
    \speakernote{Find an input to reach bug (line 9)}
...
\begin{frame}
    ...
    \script{%
        ...
        generator which is often mutational (\ie~mutates already existing inputs
        by bit flips, byte replacements, etc.) or generational random
        (\ie~generates new inputs randomly). New inputs are fed to the target
        which is monitored for unexpected behaviours such as crashes, timeouts
        and so on. Its simplicity provides high throughput in terms of generated
        inputs but unfortunately it doesn't allow BB fuzzing to explore deeper
