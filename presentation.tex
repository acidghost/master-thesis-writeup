...
\begin{frame}
    ...
    \end{itemize}
    \script<1>{%
        Prof. Barton Miller from University of Wisconsin was logged into his
        office's terminal via a dial-up modem. The heavy rain was causing noise
        on the line which would scramble his inputs before they could reach the
        other end; he noticed that these random inputs were causing receiving
        programs to crash and decided to investigate the phenomenon.
    }
    \script<2>{%
...
\end{lstlisting}
    \script{%
        ...
        number of checks the target imposes:
        \begin{itemize}
            ...
            \item{} check on markers, which are magic bytes at dynamic positions
...
\begin{frame}
    ...
    \script{%
        Black box fuzzing is the simplest form as requires no knowledge of the
        target internals. Its core component is an input generator which is
        often mutational (\ie~mutates already existing inputs by bit flips, byte
        replacements, etc.) or generational random (\ie~generates new inputs
        randomly). New inputs are fed to the target which is monitored for
        unexpected behaviours such as crashes, timeouts and so on. Its
        simplicity provides high throughput in terms of generated inputs but
        unfortunately it doesn't allow BB fuzzing to explore deeper portions of
        the target.
    }
\end{frame}
...
\begin{frame}
    ...
    \script{%
        ...
        less knowledge about the target internals to pass difficult checks on
        the input.
        \begin{itemize}
            \item{} The system described in Hybrid Fuzz Testing uses s.e.\ to
                ...
                nodes.
            \item{} Driller, on the other hand, starts off with AFL and whenever
                no new interesting inputs are generated, it uses symbolic
                execution to find an input that may allow AFL to pass a
                difficult check.
        \end{itemize}
    }
\end{frame}
