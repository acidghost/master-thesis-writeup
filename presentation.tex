...
\end{lstlisting}
    \script{%
        ...
        execution reach the bug; to achieve this, the input needs to satisfy a
        number of checks that the target imposes:
        \begin{itemize}
            ...
            \item{} checks for markers, which are magic bytes at dynamic
                positions
            \item{} and finally all those kind of checks can be nested
        \end{itemize}
...
\begin{frame}
    ...
    \script<1>{%
        ...
        communicate with each other to achieve a common goal. The motivations
        for such system are:
        \begin{itemize}
...
\begin{frame}
    ...
    \script{%
        ...
        communicate with the different fuzzers. Test cases are used as a common
        information unit that can be extracted from and injected into fuzzers as
        most of them already maintain an internal queue of inputs. The system
        requires fuzzers to implement two API primitives (extraction and
...
\begin{frame}
    ...
    \script{%
        ...
        slide. The figure shows how fuzzers broadcast info to all other fuzzers
        but the channels are mediated by the CDU.
    }
    \twocols{%
        \begin{figure}
...
\script<2>{%
    ...
    input and the winners are injected with the input. Scoring function and
    winner selection are central components defining a specific cooperative
    strategy.
}
\end{frame}
...
\script{%
    We now move on to the evaluation, but before delving into the evaluation of
    cooperation, I'm going to present an evaluation of individual fuzzers
    without cooperation.
}
...
\begin{frame}[label=coop-best]
    ...
    \script{%
        ...
        coop.\ strategies outperforms the union of fuzzers. In particular for 3
        targets, roughly $77\%$ of HDI is above zero and for objdump $95.3\%$
        is.
    }
\end{frame}
...
\begin{frame}
    ...
    \end{itemize}
    \script{%
        Regarding the evaluation of fuzzers without cooperation, on average we
        found that \fairfuzz\ performs the best; this result was unexpected and
        evaluating on a broader array of targets may yield different results. On
        the same note, repeating runs for more than 5 times may remove some
        noise and yield more decisive results. For the evaluation of
        cooperation, the observed means suggest that cooperation is beneficial
        to increase code coverage; unfortunately though, BEST does not provide
        high confidence. To try to cope with this, more, longer runs may yield
        more definitive results. Finally, cooperation proved to be beneficial in
        ...
        so a more thoughtful analysis would include data for more targets.
    }
\end{frame}
