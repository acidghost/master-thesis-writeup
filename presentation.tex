...
\ifthenelse{\equal{\HandoutMode}{handout}}{%
    ...
    \ifthenelse{\not\equal{\NotesSecondScreen}{} \and \equal{\NotesOnly}{}}{%
...
\begin{frame}
    \titlepage%
    \script{%
        Welcome everyone, my name is \myName\ and I'd like to present to you my
        Master project titled ``\myTitle: \mySubtitle''.
    }
    \speakernote[item]{welcome everyone}
    \speakernote[item]{present yourself and your work}
    \speakernote[item]{don't be nervous!}
\end{frame}
...
\begin{frame}
    ...
    \end{itemize}
    \script{%
        In particular, I designed and implemented a framework that allows
        diverse fuzzers, running in parallel, to cooperate. The evaluation of
        the effect of cooperation shows promising results both in terms of
        coverage and number of unique crashes discovered.
    }
    \speakernote[item]{designed and implemented framework}
    \speakernote[item]{evaluated cooperation promising results}
    \speakernote[item]{both terms of coverage and crashes discovered}
\end{frame}
...
\begin{frame}
    ...
    \note<2->[item]{then we'll move on to describe the CFF, with a focus on its
...
\note{Let's begin by looking at different fuzzing techniques}
...
\begin{frame}
    ...
    \end{itemize}
    \script<1>{%
        The term fuzzing was initially coined in 1988, quite literally during a
        quiet and stormy night. Prof. Barton Miller from University of Wisconsin
        was logged into his office's terminal through a dial-up modem. The heavy
        rain was causing noise on the line which would scramble his inputs
        before they could reach the other end; he noticed that these random
        inputs were causing receiving programs to crash and decided to
        investigate the phenomenon.
    }
    \script<2>{%
        Since then fuzzing has evolved into an established reliability and
        security testing practice. For example Mayhem, a system that uses
        symbolic execution and directed fuzzing to find vulnerabilities, won the
        2016 DARPA Cyber Grand Challenge, the first all-machine CTF competition.
        Moreover, fuzzing is widely used in the industry for example at
        Microsoft and Google.
    }
    \speakernote<1>[item]{coined in 1988 Prof. Barton Miller, Univ.\ of Wisconsin}
    \speakernote<1>[item]{was logged into his office terminal dial-up modem}
    \speakernote<1>[item]{heavy rain causing noise on the line}
    \speakernote<1>[item]{cause inputs to scramble and crash, investigate phenomenon}
    \speakernote<2>[item]{Mayhem -- system that uses S.E.\ + directed fuzz.\ in offence}
    \speakernote<2>[item]{CGC -- first all-machine CTF}
    \speakernote<2>[item]{Project Springfield (commercial, Microsoft)}
    \speakernote<2>[item]{OSS-Fuzz (for OSS, Google)}
\end{frame}
...
\end{lstlisting}
    \script{%
        The goal of fuzzing is, given for example the function shown, to find
        the bug on line 9. To do so, it needs to find an input that lets
        execution reach that portion of code by satisfying a number of checks
        that the target imposes:
        \begin{itemize}
            \item{} checks on metadata such as the length of the input
            \item{} checks for magic bytes, which are specific values at
                specific positions in the input
            \item{} checks for markers, which are specific values at dynamic
                positions
            \item{} and finally nested checks
        \end{itemize}
        Different fuzzing techniques tackle these challenges differently, based
        on the amount of knowledge they have of the internal structure of the
        target.
    }
    \speakernote{Find bug (line 9) by finding input to reach it}
    \speakernote[item]{metadata like length (line 3)}
    \speakernote[item]{magic bytes (line 5)}
    \speakernote[item]{markers (\ie~magic bytes at dynamic position) (line 8)}
    \speakernote[item]{nested statements (lines 5,7,8)}
    \speakernote[item]{diverse fuzz tackle challenges differently, based on knowledge}
\end{frame}
...
\begin{frame}
    ...
    \script{%
        Black box fuzzing is the simplest form of fuzzing as requires no
        knowledge of the target internals. Its core component is an input
        generator which is often mutational (\ie~mutates already existing
        inputs) or generational random (\ie~generates new inputs randomly). New
        inputs are fed to the target which is monitored for unexpected
        behaviours such as crashes, timeouts and so on. Its simplicity provides
        high throughput in terms of generated inputs but unfortunately it
        doesn't allow BB fuzzing to explore deeper portions of the target.
    }
    \speakernote[item]{simplest form, no knowledge target internals}
    \speakernote[item]{core component: mutates existing or generational}
    \speakernote[item]{new inputs fed to target, monitored for}
    \speakernote[item]{simplicity gives high throughput}
\end{frame}
...
\begin{frame}
    ...
    \script{%
        ...
        track constraints on the input along an execution path which are then
        ...
        is used by the target and unlock information that enable, for example,
        to infer an input grammar. Unfortunately, while providing great
        efficiency, white box techniques do not scale well for larger programs.
    }
\end{frame}
...
\script{%
    Now we can move on to looking at how these techniques can be used
    together
}
...
\begin{frame}
    ...
    \end{exampleblock}
    \script{%
        Intention of hybrid approaches, is to use white box fuzzing exclusively
        to help fuzzers with less knowledge about the target internals to pass
        difficult checks on the input.
        \begin{itemize}
            \item{} The system described in Hybrid Fuzz Testing uses s.e.\ to
                reach a configurable number of so called frontier nodes; then, it
                switches to black box mutational fuzzing where mutation is
                constrained so that generated inputs still reach the frontier
                nodes while exploring new paths.
            \item{} Driller, on the other hand, starts off with AFL, a popular
                coverage-based gray box fuzzer, and whenever no new interesting
                inputs are generated, it uses symbolic execution to find an
...
\begin{frame}
    {Cooperative Fuzzing}
    \script<1>{%
        Finally we can move on to coop.\ fuzzing. I use the term to refer to a
        system that runs a number of fuzzers, possibly of different kind, in
        parallel. Moreover these fuzzers communicate with each other to achieve
        a common goal. The motivations for such system are:
        \begin{itemize}
            \item{} fuzzing is non-deterministic so parallelization is favorable
...
\script{%
    With a bit of background in place, we can move to the CFF; in particular
    by looking at its design
}
...
\begin{frame}
    ...
    \script{%
        ...
        fuzzers, we have to first define an API that the system can use to
        communicate with them. Test cases are used as a common information unit
        that can be extracted from and injected into fuzzers. The system
        requires them to implement two API primitives (extraction and
        injection), plus one optional, for congestion control, that allows a
        fuzzer to signal when it is able to receive new inputs via an inject
        call.
    }
    \begin{center}
        \usebeamercolor{section in head/foot}
...
\begin{frame}
    ...
    \script{%
        Then we need a central decisional unit to mediate the exchange of info.
        To do so the system uses the API just introduced. How these API methods
        are used is defined by the specific coop.\ strategy, which we'll talk
        about in the next slide. The figure shows how fuzzers broadcast info to
        all other fuzzers but the channels are mediated by the decisional unit.
    }
    \twocols{%
        \begin{figure}
...
\script<1>{%
    With the architectural design in place, we can look at coop.\ strategies. A
    coop.\ strategy determines how info flows among fuzzers. The algorithm gives
    a template for a generic strategy.
}
\script<2>{%
    In red are highlighted the important bits, excluding congestion control
    mechanics. Whenever a new input is extracted from a fuzzer, the system
    starts a competition; all other fuzzers are scored with regards to the new
    input and the winners are injected with it. Scoring function and winner
    selection are central components defining a specific cooperative strategy.
}
\end{frame}

\subsection{System Implementation}

\script{Now let's look at the implementation of the CFF}
...
\begin{frame}
    ...
    \begin{tikzpicture}
        ...
        \begin{umlcomponent}[x=3,y=0,bottom color=bg, top color=white]{CFF}
            ...
            \umlbasiccomponent[x=4,y=2,bottom color=bg, top color=white]{Master}
            \only<3->{%
            ...
            \only<4->{%
                \umlnote[x=-2.8,y=2.6,width=4.5cm]{Driver}{%
                    \texttt{Score}: new BTS transitions
                }
                \umlnote[x=2.9,y=-0.3,width=4.6cm]{Master}{%
                    \texttt{Winner}: highest, higher than $t$
                }
            }
        \end{umlcomponent}
        \only<2->{%
    ...
    \script<2>{%
        The driver uses the API to interface with the fuzzer. As most fuzzers
        synchronize test cases periodically with the file system, our
        implementation listens for and injects new files in predetermined
        folders the fuzzer uses.
    }
    \script<3>{%
    ...
    \script<4>{%
        ...
        collected by Intel Branch Trace Store. Winner selection is implemented
        by the master and is configurable to select the highest score or those
        that are greater than a configurable threshold.
    }
\end{frame}
...
\script{%
    We now move on to the evaluation, but before looking at cooperation, I'm
    going to present an evaluation of individual fuzzers without cooperation.
}
...
\begin{frame}
    ...
    \script<1>{%
        ...
        \begin{enumerate}
            \smallitemsep
            \item{} is there a best fuzzer, in terms of code coverage, across
                different targets?
            \item{} is there a best fuzzer for each specific target?
        \end{enumerate}
        The fuzzers I evaluate, which are the same used in the evaluation of the
        CFF, are:
        \begin{itemize}
            \smallitemsep
            \item{} \aflfast~and~\fairfuzz, which are both extensions to AFL but
                with different focus and optimizations;
    ...
    \script<2>{%
        ...
        times and the mean coverage for each fuzzer is compared with that of the
        other fuzzers, for each target.
    }
\end{frame}
...
\begin{frame}[label=single-time]
    ...
    \script{%
        ...
        better than the other. To have a better understanding, besides looking
        at the final means, I used a Bayesian estimation method to evaluate the
        difference of means between two fuzzers.
    }
\end{frame}
...
\begin{frame}[label=single-best]
    ...
    \script{%
        ...
        Than, a sampling algorithm is used to produce a number of parameter
        samples which are in accordance with the data. Each figure shows the
        difference of means at each sample for the first vs.\ second fuzzer in
        order of final mean coverage. To decide whether the difference is high
        enough to declare a fuzzer better than the other, it is sufficient to
        look at the $95\%$ Highest Density Interval (\ie~the interval onto which
        $95\%$ of the most credible values lie): if it does not include zero,
        than one fuzzer is better than the other. This is the case for objdump
        and listswf (which is not shown for brevity), for which FairFuzz
        performs decisively better. The contrary is true for the other two
        targets for which is not possible to say which fuzzer is better with
        high credibility.
    }
\end{frame}
...
\script{And finally we move on to the evaluation of cooperation}
...
\begin{frame}[label=coop-best]
    ...
    \begin{itemize}
        ...
        \item{} \objdump~$\longrightarrow$ \parbox{.7\textwidth}{%
                ...
                multi vs.\ union $96.7\%$ of density is $> 0$}
    \end{itemize}
    \buttons{%
    ...
    \script{%
        ...
        final mean coverage vs.\ the union of fuzzers. Unfortunately, by looking
        at the $95\%$ HDI, we cannot say with high credibility that any of the
        coop.\ strategies outperforms the union of fuzzers. In particular the
        density is by $95.3\%$ above zero for objdump and roughly $77\%$ for the
        other targets.
    }
\end{frame}
...
\script{%
    Lastly, I'm going to present a brief analysis of the CFF in terms of crashes
}
...
\begin{frame}
    ...
    \script<1>{%
        ...
        unique path, every crashing input stored by fuzzers is fed to the target
        running through a debugger. This validates the crash before running the
        \texttt{exploitable} plugin to get its stack hash, which is the hash of
        the last calls on the stack before the crash. The stack hashes and
        relative elapsed time from 5 runs are accumulated into a single file and
        processed.
    }
    \uncover<2->{%
        \vspace{\baselineskip}
    ...
    \script<2>{%
        ...
        strat.\ finds $30\%$ more distinct unique crashes than the union of
        fuzzers and more crashes that the union doesn't find.
    }
\end{frame}
...
\begin{frame}
    ...
    \script{%
        ...
        the same bug. Of the over 25 bugs found, I was able to link 6 to a CVE
...
\begin{frame}
    ...
    \script{%
        Finally, I'm going to briefly discuss those results and conclude my
        presentation
    }
\end{frame}
...
\begin{frame}
    ...
    \end{itemize}
    \script{%
        Regarding the evaluation of fuzzers without cooperation, on average
        \fairfuzz\ performs the best; this result was unexpected and evaluating
        on a broader array of targets may yield different results. On the same
        note, repeating runs for more than 5 times may remove some noise and
        yield more decisive results. For the evaluation of cooperation, the
        observed means suggest that cooperation is beneficial to increase code
        coverage; unfortunately though, BEST does not provide high confidence to
        support this. To try to cope with this, more, longer runs may yield more
        definitive results. Finally, cooperation proved to be beneficial in
        finding unique crashes, with an increase of $30\%$ over the fuzzers
        without cooperation. Unfortunately only one target revealed crashes and
        so a more representative analysis would include data for more targets.
    }
\end{frame}
...
\begin{frame}
    ...
    \end{block}
    \script{%
        In conclusion, I presented the design and implementation of a framework
        that allows diverse, existing fuzzers to cooperate; moreover, I
        presented its evaluation as well as an evaluation of the underlying
        fuzzers, without cooperation. To further improve this work, I'd conduct
        more experiments, both in terms of number of targets and number of runs;
        use more and more diverse fuzzers; and finally use more complex
        strategies and scoring functions.
    }
\end{frame}
